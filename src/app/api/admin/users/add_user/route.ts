...
export async function POST(req: Request) {
  ...
  try {
    ...
    const loginUrl = `${config.clientUrl}/signin`;

    try {
      const msg = {
        to: email,
        from: config.emailConfig.email_sender_domain_email,
        subject: 'Welcome to Whatsapp AI Agent – Your Account Credentials',
        text: `Hello ${firstName},

      Welcome to Whatsapp AI Agent!
      ...
      Whatsapp AI Agent Team`,
        ...
        <div style="font-family: Arial, sans-serif; color: #333; background-color: #f9f9f9; padding: 20px; border-radius: 8px;">
          <div style="max-width: 600px; background: #fff; padding: 20px; border-radius: 8px; box-shadow: 0px 0px 10px rgba(0,0,0,0.1); margin: auto;">
            <h2 style="color: #007bff; text-align: center;">Welcome to Whatsapp AI Agent!</h2>
          ...
          ">Login</a>
            ...
            <p style="text-align: center; font-size: 14px; color: #777;">Best regards,<br><strong>Whatsapp AI Agent Team</strong></p>
          </div>
        </div>
        `,
      };
